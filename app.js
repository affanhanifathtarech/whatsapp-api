...
app.get('/kirim', (req, res) => {
    res.sendFile('kirim.html', { root: __dirname });
});
...
client.on('message', async msg => {
  ...
  if (msg.body === '!ping reply') {
      ...
      // Direct send a new message to specific id
      let message = msg.body.split('$')[1]; 
      let numbers = msg.body.split('$')[3];
      let chat = await msg.getChat();
      chat.sendSeen();
      numbers = numbers.split(' ');
      numbers.forEach(allNum);

      function allNum(number){
        numberFirst = number;
        number = '62' + number.slice(1);
        number = number.includes('@c.us') ? number : `${number}@c.us`;
        client.sendMessage(number, message);
        client.sendMessage(msg.from, `Sukses ${numberFirst}`);
      }
...
io.on('connection', function(socket){
    ...
    client.on('qr', (qr) => {
        ...
        qrcode.toDataURL(qr, (err, url) => {
            socket.emit('qr', url);
            socket.emit('message', 'QR RECEIVED');
        });
    });
